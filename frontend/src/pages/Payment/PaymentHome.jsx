      ...
      if (currentUser) {
        try {
          const response = await axios.get(
            `http://localhost:3000/api/payments/totalDueAmount/${currentUser._id}`
          );
      ...
      <Scroll>
        <div className="max-w-lg w-full bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="flex justify-center py-4 ">
            <img src={garbage_money} alt="Garbage" className="h-40 w-auto" />
          </div>

          {/* Total Due Amount Section */}
          {!isOverDue && (
            <div className="bg-yellow-200 text-center py-4 rounded-xl mx-4">
          ...
          {isOverDue && (
            <div className="bg-red-200 text-center py-4 px-4 rounded-xl mx-4">
              <p className="text-xl font-bold text-red-600">
                Payment Overdue!!
              </p>
              <p className="text-sm text-gray-700">
                Dear customer, unfortunately one or more of your payments are
                overdue.
              </p>
              <p className="text-sm text-gray-700">
